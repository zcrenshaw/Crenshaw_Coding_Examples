...
//Mercedes Wentworth-Nice and I worked on significant portions of this project together
//Therefore, some of the code may appear rather similar 
...
{
	if (bc == NULL) {
		fprintf(stderr,"deanon_preprocess: no blockchain to process");
		exit(1);
	}
	bc->deanon = deanon_new();
	block_t *tmp_b = bc->blocks;
	while (tmp_b != NULL) {
		trans_t *tmp_t = tmp_b->trans;
		while (tmp_t != NULL) {
			add_associations(tmp_t->inputs,tmp_t->num_inputs,bc->deanon);
			tmp_t = tmp_t->next;
		}
		tmp_b = tmp_b->next;
	}
...
double cluster_balance(cluster_t* cluster, blockchain_t* bc)
{
    ...
    while (cluster != NULL) {
	    double out_bal = 0;
	    if (get_balance(cluster->addr,&out_bal,bc->addr_bal))
		    cbal += out_bal;
	    cluster = cluster->next;
    }
